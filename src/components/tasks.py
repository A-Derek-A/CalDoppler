...
@dataclass
class CalReceivedSignalTask(Task):
    """计算接收到的信号任务。"""
    ...
    def run(self):
        ...
        for _ in range(self.n_samples):
            la, lo = sample_point_in_spherical_cap(lat, lon, psi)
            gs = wgs84.latlon(la, lo)
            doppler, signal = self.sat.get_doppler(self.time, gs, False, mode = 2)
            results.append((la, lo, doppler + self.offset, signal))

        # 写入文件
        inter_dir = Path(__file__).parent.parent.parent / "data" / "intermediate"
        inter_dir.mkdir(parents=True, exist_ok=True)
        with open(inter_dir / f"{self.task_id}.txt", "w") as f:
            for la, lo, doppler, signal in results:
                f.write(f"{la},{lo},{doppler},{signal}\n")
        return results

@dataclass
class MergeReceivedSignalTask(Task):
    def run(self):
        """合并所有子任务的结果。"""
        inter_dir = Path(__file__).parent.parent.parent / "data" / "intermediate"
        final_dir = Path(__file__).parent.parent.parent / "data" / "final"
        final_dir.mkdir(parents=True, exist_ok=True)
        if (final_dir / f"result-received-signal.txt").exists():
            (final_dir / f"result-received-signal.txt").unlink()
        with open(final_dir / f"result-received-signal.txt", "a") as f:
            for file in inter_dir.glob("*.txt"):
                with open(file, "r") as f_in:
                    for line in f_in:
                        f.write(line)
